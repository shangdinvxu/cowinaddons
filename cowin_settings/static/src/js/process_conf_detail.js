...
odoo.define('cowin_settings.process_conf_detail', function (require) {
    ...
    var ProcessConfDetail = Widget.extend({
        ...
        show_approval_flow_wrap:function (e) {
            ...
            if($(target).parents('tr').find('.tache_name').text()=='信息登记'||$(target).parents('tr').find('.tache_name').text()=='项目退出会议表决票'||$(target).parents('tr').find('.tache_name').text()=='投资决策委员会决议'||$(target).parents('tr').find('.tache_name').text()=='投委会会议表决票'){
                var operate = false;
            }else {
                var operate = true;
            }
