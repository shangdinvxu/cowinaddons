...
class Cowin_sub_project_approval_flow_settings(models.Model):
    ...
    def send_approval_flow_settings_node_msg(self, status=2, is_current_or_next=True):
        ...
        tmp2[u'approval_role_person'] = self.env.user.employee_ids[0].name_related
        ...
        # 在审核操作时候  时记录审批结果,审批意见
        tmp2[u'approval_result'] = u''
        tmp2[u'approval_opinion'] = u''
        ...
        if status == 1:   # 提交的操作
            ...
            tmp2[u'operation'] = u'审核'
            if self.sub_pro_approval_flow_settings_record_ids:
                tmp2[u'approval_result'] = self.sub_pro_approval_flow_settings_record_ids[-1].approval_result
                tmp2[u'approval_opinion'] = self.sub_pro_approval_flow_settings_record_ids[-1].approval_opinion
        ...
        if status == 6:  # 投票的消息
            ...
            # channel_entity.message_post(json.dumps(tmp2), message_type='comment', subtype='mail.mt_comment', subtype_id=subtype_id, partner_ids=partner_ids)
        else:  # 普通消息
            ...
            def send_message_():
                if is_current_or_next:  # 当前发送消息
                    ...
                    channel_entity.message_post(json.dumps(tmp2), message_type='comment', subtype='mail.mt_comment', subtype_id=subtype_id,
